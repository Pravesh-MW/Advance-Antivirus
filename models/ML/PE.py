...
class PE:
    def __init__(self):
        ...
        self.clf = joblib.load(f"{parent_directory}\\classifier.pkl")
        
        self.features = pickle.loads(open(os.path.join(f"{parent_directory}\\features.pkl"),'rb').read())
    ...
    def test(self, path):
        data = self.extract_infos(path)

        #matching it with the features saved in features.pkl
        pe_features = list(map(lambda x:data[x], self.features))
...
#     pe = PE()
#     pe.test("C:\\Users\\Asus\\Downloads\\python-3.12.0-amd64.exe")
